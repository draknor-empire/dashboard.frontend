...
export class DashboardComponent {
  ...
  calculateNextDueDate(currentDate: Date, billFrequency: string, typicalDueDay: number): Date {
    ...
    // If the current day of the month is after the typical due day, increment the month
    if (nextDueDate.getDate() > typicalDueDay) {
        if (timeUnit === 'M') {
    ...
    // Set the day of the month to the typical due day
  ...
  chaseCardsArray: Array<Array<string>> = [
    ['Freedom Sapphire', ' ',this.calculateNextDueDate(this.currentDate, 'monthly', 15).toLocaleDateString() , ' '],
    ['Freedom Unlimited', '', this.calculateNextDueDate(this.currentDate, 'monthly', 15).toLocaleDateString(), ' ']
  ];
  ...
  capitalOneDataArray = [['Capital One Platinum', 'Capital One Cash']];
  ...
  consolidatedMedicalSupplyHeadingArray = [
    'Account Type',
    'Due Date',
    'IsAutomated',
  ];
  consolidatedMedicalSupplyDataArray = [
    ['Consolidated Medical Supply: Pharmacy'],
  ];
}
